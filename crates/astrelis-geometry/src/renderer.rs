...
use crate::pipeline::{create_fill_pipeline, create_projection_bind_group_layout, create_stroke_pipeline};
...
impl GeometryRenderer {
    ...
    pub fn new(context: Arc<GraphicsContext>) -> Self {
        ...
        let projection_buffer = context.device().create_buffer(&wgpu::BufferDescriptor {
            label: Some("Geometry Projection Buffer"),
        ...
        let projection_bind_group = renderer.create_bind_group(
            Some("Geometry Projection Bind Group"),
        ...
        // Create pipelines
        let fill_pipeline = create_fill_pipeline(&renderer, &projection_bind_group_layout);
        let stroke_pipeline = create_stroke_pipeline(&renderer, &projection_bind_group_layout);

        // Create instance buffers
        let fill_instances = InstanceBuffer::new(context.device(), Some("Fill Instances"), 256);
        let stroke_instances = InstanceBuffer::new(context.device(), Some("Stroke Instances"), 256);
    ...
    pub fn render(&mut self, pass: &mut wgpu::RenderPass, viewport: Viewport) {
        ...
        enum RenderOp {
            ...
            ResetScissor,
            DrawFill { index_start: u32, index_count: u32, instance_idx: u32 },
            DrawStroke { index_start: u32, index_count: u32, instance_idx: u32 },
        }
        ...
        for cmd in &self.draw_commands {
            match cmd {
                DrawCommand::Fill { mesh, color, offset } => {
                ...
                DrawCommand::Stroke { mesh, color, width, offset } => {
        ...
        for op in ops {
            match op {
                ...
                RenderOp::ResetScissor => {
                    pass.set_scissor_rect(0, 0, physical_size.width as u32, physical_size.height as u32);
                }
                RenderOp::DrawFill { index_start, index_count, instance_idx } => {
                    if let (Some(vbo), Some(ibo)) = (&self.fill_vertex_buffer, &self.fill_index_buffer) {
                ...
                RenderOp::DrawStroke { index_start, index_count, instance_idx } => {
                    if let (Some(vbo), Some(ibo)) = (&self.stroke_vertex_buffer, &self.stroke_index_buffer) {
