...
///
/// This explicit lifecycle call remains available when the `enabled`
/// feature is off, although the profiling macros produce no events.
#[inline]
pub fn new_frame() {
    profiler::frame_mark();
}
...
pub mod private {
    ...
    pub use crate::profiler::{ScopeGuard, enter_scope, record_counter_shim, record_counter_value};
}
...
mod tests {
    ...
    fn scope_spans_are_recorded() {
        ...
        assert!(
            has_span,
            "expected at least one recorded span after frame_mark"
        );
    }
}
