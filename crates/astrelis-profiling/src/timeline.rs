...
impl ThreadStream {
    ...
    pub fn spans_in_window(
        ...
        let start_idx = self.spans.partition_point(|s| s.end_ns <= visible_start_ns);
        self.spans
            .iter()
            .skip(start_idx)
...
impl Timeline {
    ...
    ///
    /// Span streams are sorted by `end_ns`, and counter streams by
    /// `ts_ns`, so the index of the first kept element can be
...
mod tests {
    ...
    fn evict_keeps_span_whose_end_equals_cutoff() {
        ...
        assert_eq!(
            spans.len(),
            2,
            "spans whose end_ns == cutoff must be retained"
        );
    }
