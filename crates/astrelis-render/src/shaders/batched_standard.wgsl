// Batched standard shader for Tier 1 (Direct) and Tier 2 (Indirect).
//
// Uses a single per-texture bind group (group 0) and a projection uniform (group 1).
// Supports three draw types via the draw_type instance field:
//   0 = Solid quad (SDF rounded rect)
//   1 = Text glyph (R8 atlas alpha × color)
//   2 = Image (RGBA texture × tint)

// Bind group 0: per-texture
@group(0) @binding(0) var bound_texture: texture_2d<f32>;
@group(0) @binding(1) var bound_sampler: sampler;

// Bind group 1: projection uniform
@group(1) @binding(0) var<uniform> projection: mat4x4<f32>;

// Vertex input: unit quad (positions 0..1 mapped to triangle strip)
struct VertexInput {
    @location(0) position: vec2<f32>,
    @location(1) tex_coords: vec2<f32>,
}

// Instance input: matches UnifiedInstance layout
...
struct VertexOutput {
    ...
    @location(2) local_pos: vec2<f32>,      // local position within quad (0..size)
    ...
    @location(7) world_pos: vec2<f32>,       // for clip rect test
...
fn vs_main(v: VertexInput, i: InstanceInput) -> VertexOutput {
    ...
    // Override Z with instance depth for depth testing
    projected.z = i.z_depth * projected.w;

    out.clip_position = projected;

    // Interpolate UVs between uv_min and uv_max based on unit quad position
...
// p: point relative to rect center, b: half-size, r: corner radius
...
fn fs_main(in: VertexOutput) -> @location(0) vec4<f32> {
    ...
    if in.draw_type == 0u {
        ...
        // --- TEXT GLYPH ---
        let atlas_alpha = textureSample(bound_texture, bound_sampler, in.tex_coords).r;
        if atlas_alpha < 0.01 {
            discard;
        }
        ...
        // --- IMAGE (draw_type == 2) ---
        let tex_color = textureSample(bound_texture, bound_sampler, in.tex_coords);
