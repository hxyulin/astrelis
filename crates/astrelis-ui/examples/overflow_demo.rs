...
fn main() {
    ...
    run_app(|ctx| {
        ...
        // Create UI system from window to inherit format configuration
...
fn build_overflow_ui(ui: &mut UiSystem, window: &RenderWindow) {
...
fn add_scrollbar_panel(ui: &mut UiSystem, colors: &ColorPalette) {
    ...
    // --- Title ---
    let title = Text::new("Scrollbar Widgets").size(16.0).color(colors.info);
...
fn build_overflow_hidden_panel(root: &mut UiBuilder, colors: &ColorPalette) -> NodeId {
    ...
    root.container()
        ...
        .child(|root| {
            ...
            for i in 1..=20 {
                let color = if i <= 5 { text_primary } else { text_secondary };
