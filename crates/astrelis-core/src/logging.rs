...
//!
//! Provides [`init_default`] to install a [`tracing_subscriber::fmt`] subscriber
...
///
/// Uses [`tracing_subscriber::fmt`] with an [`EnvFilter`](tracing_subscriber::EnvFilter)
    ...
    let filter = EnvFilter::try_from_default_env().unwrap_or_else(|_| {
        EnvFilter::new(
            "warn,\
             astrelis_core=info,\
             astrelis_gpu=info,\
             astrelis_window=info,\
             astrelis_assets=info,\
             astrelis_text=info,\
             astrelis_text_wgpu=info",
        )
    });
