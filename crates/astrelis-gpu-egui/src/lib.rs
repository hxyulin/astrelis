//! egui integration for the `astrelis-gpu` backend.
//!
...
//! let egui = EguiIntegration::new(&gpu, surface_format);
//!
...
//!     ui.label("Hello!");
//! });
//! egui.end_frame_and_render(&gpu, &mut encoder, &view, screen_desc, Some(&window));
//! ```
...
/// Integrates egui with the astrelis windowing and GPU rendering backends.
///
...
impl EguiIntegration {
    ...
    ///
    /// - `gpu`: The GPU backend, used to access the raw wgpu device.
    ...
    ///
    /// - `gpu`: The GPU backend for buffer/texture updates.
    ...
    pub fn end_frame_and_render(
        ...
        let wgpu_device = gpu.raw_device();
        let wgpu_queue = gpu.raw_queue();
    ...
    pub fn register_texture(
        ...
        self.renderer
            .register_native_texture(gpu.raw_device(), texture_view, filter)
    }
